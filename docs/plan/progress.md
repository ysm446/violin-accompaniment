...
- パッケージ版の楽譜は `resources/scores/`、core は `resources/core/violin_core.exe`。対象曲は `ui/electron/main.cjs` の `SCORE_MIDI` と `ui/src/main.ts` の `SCORE_URL` で対にして指定する(Phase 1 以降で曲選択 UI にする)。
