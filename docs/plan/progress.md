...
- Phase 4: テンポ推定(カルマン)+ 先読みスケジューリング。Phase 5 の残り: 曲頭のロックイン、楽譜内の反復楽句、ミスへの耐性。詳細は [plan.md](plan.md)。
...
- 追従器のパラメータは `OnlineFollower.__init__` の既定値(onset_weight 0.5 / onset_bonus 1.0 / transient_damp 0.5 / near_eps 0.05 / measurement_gain 0.25 / jump_margin 6 / restart_penalty 15 / snap_min_strength 0.5 / snap_min_interval 0.5)。実演奏の結果はパラメータに敏感(記録が 1 つしかない)なので、変更時は `tools/eval_follower.py` の合成と実演奏の両方を見る。
