...
6. 追従 ON の記録 3 本で判明した「長い演奏での崩壊」への対応を続ける(進捗は progress.md)。記録 4 本を回帰評価ツールにする。
7. 伴奏の開始条件(`follow_settings`: 1 秒間ずっと確実・一意性 ≥ 0.3)の緩和を検討する。通しモードでは楽譜全体の一意性は不要なので、一意性の判定も ±16 拍の範囲に限定する案(反復の多い曲で開始しやすくなる)。
8. 通し/どこからでも の切り替えと副仮説の可視化を UI に。楽譜内で繰り返される楽句(冒頭のリトルネロ等)は設計上一意にならないので、曲構造の指定か「直前の位置に近い候補を優先」する案。
